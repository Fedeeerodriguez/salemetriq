      ...
      <div className="flex flex-col gap-3">
        ...
        {listas.map((l) => (
          <div key={l.id} className="card p-4 flex items-center justify-between gap-3">
            <Link to={`/listas/${l.id}`} className="flex items-center gap-3 min-w-0 flex-1 group">
            ...
            <div className="flex items-center gap-1">
              ...
              <Link to={`/listas/${l.id}`} className="icon-btn"><ChevronRight size={18} /></Link>
            </div>
          </div>
        ))}
      </div>
