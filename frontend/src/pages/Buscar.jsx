...
export default function Buscar() {
  ...
  // Al elegir nicho, si el ángulo es hashtag y hay hashtags, precargo el primero.
  useEffect(() => {
  ...
  async function lanzar(e) {
    ...
    try {
      const { data } = await api.post("/busqueda", {
        angulo, query, nicho_id: nichoId || null, filtros: limpiarFiltros(),
      });
      setJob(data);
      poll.current = setInterval(async () => {
        const { data: j } = await api.get(`/busqueda/${data.id}`);
        setJob(j);
    ...
    <div className="max-w-3xl">
      ...
      {job && (
        <div className="card p-4 mt-4 flex items-center justify-between gap-3">
          ...
          {job.estado === "ok" && (
            <button onClick={() => navigate(`/perfiles${nichoId ? `?nicho=${nichoId}` : ""}`)} className="btn-ghost">
              Ver perfiles →
            </button>
          )}
        </div>
